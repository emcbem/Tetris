...
namespace TetrisTests.StepDefinitions
{
	...
	public sealed class TetroisStepDefinitions
	{
		...
		{
			_scenarioContext.Get<Board>("board").FindFilledLine().Should().NotBe(-1);
		}

		[When(@"the found line is removed")]
		public void WhenTheFoundLineIsRemoved()
		{
			var board = _scenarioContext.Get<Board>("board");
			board.RemoveLine(board.FindFilledLine());
		}

		[Then(@"the board should recognize there is no line to remove")]
		public void ThenTheBoardShouldRecognizeThereIsNoLineToRemove()
		{
			_scenarioContext.Get<Board>("board").FindFilledLine().Should().Be(-1);
		}

		[When(@"the found lines are removed")]
		public void WhenTheFoundLinesAreRemoved()
		{
			var board = _scenarioContext.Get<Board>("board");
